...
namespace Lambdas {
    ...
    static void test_01()
    {
        ...
        std::vector<int>::iterator  pos2 = vec.end();
        while (pos != pos2) {
        ...
        // geht nur für SEQUENTIELLE Container
        for (int i = 0; i < vec.size(); i++) {
            std::cout << "n: " << vec[i] << std::endl;
        }
    ...
    static void test_04() {
        ...
        // berechnung:  hmmm, okay,  auf- oder absteigend
        auto flag = true;

        std::sort(
            vec.begin(),
            vec.end(),
            [&flag] (int n1, int n2) -> bool {
    ...
    static void test_06() {
        ...
        auto lambdaWithThreeParams0{
            [](int x, int y, int z) {
                return x + y + z;
            }
        };

        int (*fp) (int, int, int) = nullptr;  // NULL
